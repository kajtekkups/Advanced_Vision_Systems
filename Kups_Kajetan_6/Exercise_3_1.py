...
if __name__ == "__main__":
    ...
    # Initialize the SGBM object
    stereo_sgbm = cv2.StereoSGBM_create(
        minDisparity=0,
        numDisparities=16*10,
        blockSize=11,
        P1=8*3*5**2,    # Penalty on small disparity changes
        P2=32*3*5**2,   # Penalty on large disparity changes
        disp12MaxDiff=1,
        uniquenessRatio=10,
    ...
    # Compute disparity
    disparity_sgbm = stereo_sgbm.compute(dst_L, dst_R)

    # Normalize for visualization
    disparity_sgbm_vis  = cv2.normalize(disparity_sgbm, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    disparity_sgbm_vis  = np.uint8(disparity_sgbm_vis )
    disparity_sgbm_vis  = cv2.applyColorMap(disparity_sgbm_vis , cv2.COLORMAP_HOT)

    cv2.imshow('Semi-Global Matching Disparity', disparity_sgbm_vis )
