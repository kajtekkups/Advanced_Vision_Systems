...
def correct_img(img_num ):
    ...
    org_image_L = cv2.imread( image_dir + f"left_{img_num:02d}.png")
    org_image_R = cv2.imread( image_dir + f"right_{img_num:02d}.png")

    dst_L = cv2.remap(org_image_L, map1_left, map2_left, cv2.INTER_LINEAR)
    dst_R = cv2.remap(org_image_R, map1_right, map2_right, cv2.INTER_LINEAR)

    return dst_L, dst_R, org_image_L, org_image_R


if __name__ == "__main__":
    
    # dst_L, dst_R, org_image_L, org_image_R = correct_img(1)
    # cv2.imwrite('Kups_Kajetan_6/dst_L.png', dst_L)
    # cv2.imwrite('Kups_Kajetan_6/dst_R.png', dst_R)
    # cv2.imwrite('Kups_Kajetan_6/org_image_L.png', org_image_L)
    # cv2.imwrite('Kups_Kajetan_6/org_image_R.png', org_image_R)
    
    dst_L = cv2.imread('Kups_Kajetan_6/dst_L.png')
    dst_R = cv2.imread('Kups_Kajetan_6/dst_R.png')
    org_image_L = cv2.imread('Kups_Kajetan_6/org_image_L.png')
    org_image_R = cv2.imread('Kups_Kajetan_6/org_image_R.png')

    dst_L = cv2.cvtColor(dst_L, cv2.COLOR_BGR2GRAY).astype(np.uint8)
    dst_R = cv2.cvtColor(dst_R, cv2.COLOR_BGR2GRAY).astype(np.uint8)
    
    # Initialize the block matching object
    stereo_bm = cv2.StereoBM_create(numDisparities=16*10, blockSize=13) 

    # Compute disparity
    disparity_bm = stereo_bm.compute(dst_L, dst_R)

    # Normalize for visualization
    disparity_bm_vis = cv2.normalize(disparity_bm, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    disparity_bm_vis = np.uint8(disparity_bm_vis)
    disparity_bm_vis = cv2.applyColorMap(disparity_bm_vis, cv2.COLORMAP_HOT)

    cv2.imshow('Block Matching Disparity', disparity_bm_vis)
    cv2.imshow('original', org_image_L)
    cv2.waitKey(0)
