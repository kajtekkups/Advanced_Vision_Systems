...
def display_surrounding_rectangle(binary_img):
    ...
    # drawing a bbox
    cv2.rectangle( binary_img, (stats[pi, 0], stats[pi, 1]), (stats[pi, 0] + stats[pi, 2], stats[pi, 1] + stats[pi, 3]), (255 ,0 ,0), 2)
    # print information about the field and the number of the largest element
    cv2.putText(binary_img, "%f" % stats[pi ,4], (stats[pi, 0], stats[pi, 1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0))
    cv2.putText(binary_img, "%d" % pi, (int(centroids[pi, 0]), int(centroids[pi, 1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0))

    # cv2.imshow("subtraction", np.uint8( labels / retval * 255))
    cv2.imshow("2", binary_img)
    cv2.waitKey()
...
for i in range(340, 1100, 1) :
    ...
    Labled = display_surrounding_rectangle(binary_img)
