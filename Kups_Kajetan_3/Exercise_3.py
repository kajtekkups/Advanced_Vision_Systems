...
images_dir = parent_dir / "zaw_avs_materials" / "lab02_cfd" / "office" / "input"
...
class ObjectDetection:
    ...
    def delet_object_pixels(self, previos_img, current_img, binary_mask):
        result = np.where(binary_mask == 255, previos_img, current_img)
        
        return result
    

    def update_background_model_mean(self, current_img, binary_mask):
        current_img = self.delet_object_pixels(self.previous_background_model_mean, current_img, binary_mask)
    ...
    #     self.background_model_median = self.a* current_img + (1- self.a)* self.previous_background_model_median
    ...
    def alghoritm(self):
        self.previous_background_model_mean = self.read_frame(139)
        ...
        for i in range(140, 1100, 1) :
            ...
            # median_sub = cv2.subtract(self.update_background_model_median, current_frame)
            ...
            binary_image_original = np.where(mean_sub > 30, 255, 0).astype(np.uint8)
